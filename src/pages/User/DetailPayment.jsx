...
//   const qrisImageRef = useRef(null);
...
//       link.href = url;
//       link.download = `qris-payment-${order?.order_code || 'order'}.png`;
...
//       toast.error("Gagal mendownload QRIS");
//     } finally {
//       setIsDownloadingQris(false);
//     }
//   };

//   // Fungsi untuk menyalin URL QRIS ke clipboard
//   const copyQrisUrl = async () => {
//     try {
//       // Jika QRIS adalah gambar lokal, konversi ke base64
//       const response = await fetch(qris);
//       const blob = await response.blob();
//       const reader = new FileReader();
...
//           .then(() => {
//             toast.success("QRIS berhasil disalin ke clipboard!");
//           })
//           .catch(() => {
...
//     } catch (error) {
//       console.error("Error copying QRIS:", error);
...
//                     <div className="relative overflow-hidden rounded-xl border-2 border-dashed border-green-300 bg-green-50 p-4 flex flex-col items-center">
//                       <div className="relative w-full max-w-xs">
//                         <img
//                           ref={qrisImageRef}
//                           src={qris}
//                           alt="QRIS"
//                           className="w-full h-auto mx-auto cursor-pointer transform transition-transform duration-300 group-hover:scale-105"
...
//                           <p className="text-xs bg-black bg-opacity-50 text-white px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-300">
...
//                     {/* QRIS Action Buttons - Hanya Download */}
...
//                   </h4>
//                   <ol className="text-sm text-yellow-700 space-y-2 list-decimal pl-4">
//                     <li>Buka aplikasi mobile banking atau e-wallet Anda</li>
//                     <li>Pilih menu "Scan QRIS" atau "QRIS"</li>
//                     <li>Arahkan kamera ke kode QR di atas</li>
//                     <li>Pastikan nominal yang tertera sesuai: {formatCurrency(order.total)}</li>
//                     <li>Konfirmasi pembayaran</li>
//                     <li>Screenshot bukti pembayaran dan upload di form di atas</li>
//                   </ol>
//                 </div>
//                 </div>
//               // </div>
...
//       {isQrisModalOpen && (
//         <div className="w-full h-full overflow-x-scroll bg-white">
//         <div 
//           className="fixed overflow-x-scroll inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 p-4"
...
//             <button
//               onClick={() => setIsQrisModalOpen(false)}
//               className="absolute top-4 right-4 md:top-8 md:right-8 text-white hover:text-gray-300 p-2 rounded-full hover:bg-gray-800 transition z-10"
//             >
...
//                 {/* QRIS Image Full Size dengan scroll jika terlalu besar */}
//                 <div className="relative max-w-[90vw] max-h-[90vh] overflow-auto bg-white rounded-lg p-2">
//                   <img
//                     src={qris}
//                     alt="QRIS Payment"
...
//                 {/* Action Buttons */}
//                 {/* <div className="flex flex-wrap gap-3 mt-6 w-full max-w-md justify-center">
//                   <button
//                     onClick={downloadQrisImage}
//                     disabled={isDownloadingQris}
...
//                     className="flex items-center justify-center gap-3 px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-medium"
//                   >
...
//                   <button
//                     onClick={() => setIsQrisModalOpen(false)}
//                     className="flex items-center justify-center gap-3 px-6 py-3 bg-gray-800 text-white rounded-xl hover:bg-gray-900 transition font-medium"
//                   >
//                     <X className="w-5 h-5" />
//                     Tutup
//                   </button>
...
import LoadingSpinner from "../../components/LoadingSpinner.jsx";
import useOrderStore from "../../stores/useOrderStore.js";
import usePaymentStore from "../../stores/usePaymentStore.js"; // Import usePaymentStore
import qris from "../../assets/images/qris.jpeg";
...
const PaymentDetail = () => {
  ...
  // Gunakan fungsi dari store yang sudah ada
  ...
  // Cek apakah pesanan bisa dibatalkan (hanya status pending)
  const canCancelOrder = order.payment_status === 'pending' && order.status !== 'cancelled';
            ...
            <div className="flex items-center gap-3">
              ...
              {canCancelOrder && order.status !== 'completed' && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">Batalkan Pesanan</span>
                </button>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                {/* Tombol Batalkan Pesanan di sini juga */}
                {canCancelOrder && (
                  <button
                    onClick={() => setShowCancelModal(true)}
                    className="w-full flex items-center justify-center gap-2 py-3 border border-red-300 text-red-600 rounded-lg hover:bg-red-50 transition font-medium"
                  >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-red-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-red-600">Kode Order</p>
                      <p className="font-medium">{order.order_code}</p>
                    </div>
                    <div>
                      <p className="text-sm text-red-600">Nama WBP</p>
                      <p className="font-medium">{order.order_data[0]?.wbp_name || '-'}</p>
                    </div>
                    <div>
                      <p className="text-sm text-red-600">Total Pembayaran</p>
                      <p className="font-bold text-lg">{formatCurrency(order.total)}</p>
                    </div>
                    <div>
                      <p className="text-sm text-red-600">Status Pembayaran</p>
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800">
                        {order.payment_status === 'pending' ? 'Menunggu Pembayaran' : 'Pending'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {order.payment_method && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {order.payment_method === 'transfer' ? 'Transfer Bank' : order.payment_method.toUpperCase()}
                        </span>
                      </div>
                    )}
                    {order.created_at && (
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{order.items.length} produk</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        Catatan wajib diisi. Alasan ini akan dicatat dalam riwayat pesanan.
                      </p>
                    </div>
                  </div>
                </div>
